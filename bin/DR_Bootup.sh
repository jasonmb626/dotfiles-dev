...
id=$(task add +"%CUST_MEETING" proj:mtg.cust.nucleus.blocker sched:sod+8.5h unt:sched+1h "$(date +%Y-%m-%d) - Nucleus Blocker" | grep -Po '\d+')
...
id=$(task add +"%CUST_MEETING" proj:mtg.cust.nucleus.scrum sched:sod+13h unt:sched+1h "$(date +%Y-%m-%d) - Nucleus Scrum" | grep -Po '\d+')
